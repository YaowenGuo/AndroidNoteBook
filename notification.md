...
有些文章会把应用内的提醒称为应用内通知。这里只是把它当做提醒。不归为通知类型。
## 用户体验设计
...
一个典型的事例是。Netflix不会在新节目或剧集发布时向每个用户发送通知，而是跟踪每个用户一直观看的特定节目，并且只在他们最喜欢的节目之一有新剧集可用时向用户发送通知。
...
清晰、准确和简洁的文本使通知消息更可用，更容易获得用户的信赖。不要用不完整的消息来挫伤用户。通知文本应在通知块中完全呈现。
